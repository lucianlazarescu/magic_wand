...
const int R_IS  = 4;
const int L_IS  = 3;
const int LIMIT_SWITCH_UP = 1;
const int LIMIT_SWITCH_DOWN = 0;
...
const char* labels[label_count] = {"0" /*horisontal*/, "1"/*up vertical */, "2"/*down vertical moves*/};

BTS7960 motor_vertical(L_EN, R_EN, L_PWM, R_PWM);
...
void setup() {
  ...
  pinMode(PIN_POWER_MOSFET,OUTPUT);
  digitalWrite(PIN_POWER_MOSFET,LOW);
...
void loop() {
  ...
  if (!digitalRead(LIMIT_SWITCH_UP)) {
    // Limit switch up a fost apăsat, oprește motorul
    digitalWrite(PIN_POWER_MOSFET, LOW);
    motor_vertical.stop();
  }

  // Verificare limit switch down
  if (!digitalRead(LIMIT_SWITCH_DOWN)) {
    // Limit switch down a fost apăsat, oprește motorul
    digitalWrite(PIN_POWER_MOSFET, LOW);
    motor_vertical.stop();
  }
  ...
  if (done_just_triggered) {
    ...
    if( labels[max_index][0]=='0'  ){//get 0 miscare orizontala
      MicroPrintf("Move HOR");
      digitalWrite(PIN_POWER_MOSFET,HIGH);
      motor_vertical.stop();
    }
    else if(labels[max_index][0]=='1' ){
      MicroPrintf("Move vertical sus");
      digitalWrite(PIN_POWER_MOSFET,LOW);
      motor_vertical.pwm = 255;                         //Set the speed, by default the speed is set to 255 you can change it 
      motor_vertical.front();                         //front functions should turn the motor in clockwise direction
    }
    else if(labels[max_index][0]=='2' ){
      MicroPrintf("Move vertical jos");
      digitalWrite(PIN_POWER_MOSFET,LOW);
      motor_vertical.pwm = 255;                         //Set the speed, by default the speed is set to 255 you can change it 
      motor_vertical.back();                         //front functions should turn the motor in clockwise direction
    }
    else{
      digitalWrite(PIN_POWER_MOSFET,LOW);
      motor_vertical.stop();
